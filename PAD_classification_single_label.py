...
#读取文件
path='G:\\2018hanjia\\feature\\2018-12-23_20_39_40_120_features_with_D&feature.csv'
...
rfecv = RFECV(estimator=clf, step=1, cv=StratifiedKFold(7),
              scoring='accuracy')
...
#        
        ...
        train_pred = clf.predict(X_test)  
        train_acc=[]
        #下一句range后的数字为类别总数，酌情更改
        for kk in range(2):
            if not sum (train_pred==1)==0:
                train_acc.append(sum((train_pred == y_test)*(train_pred== kk))/sum (train_pred==kk))
            else:                    
                train_acc.append(0)
...
#        train_acc=[]
#        for kk in range(2):
#            if not sum (train_pred==kk)==0:
#                train_acc.append(sum((train_pred == y_test)*(y_test== kk))/sum (y_test==kk))
#            else:                    
#                train_acc.append(0)
#                
#        scores.append(train_acc)
...
###    KFold(n_splits=2, random_state=None, shuffle=False)
    print(name,'\t--> ',np.mean(scores,axis=0))
